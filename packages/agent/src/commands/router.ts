  ...
  async execute(cmd: CommandMatch, msg: IncomingMessage): Promise<void> {
    ...
    switch (cmd.command) {
      ...
      case "/workspace": {
        ...
        try {
          // Resolve which session we intend to resume (explicit arg, else latest for the path).
          const resumeId = wsSessionId || this.workspaceManager.getLatestSessionId(wsPath);

          // Takeover gate: refuse to resume a session that is still held by a
          // live terminal process. The user must exit it in the terminal first,
          // otherwise two processes would write to the same session history.
          if (resumeId) {
      ...
      case "/handback": {
        ...
        if (!ws || !ws.sessionId) {
          await this.transport.sendText(chatId, "❌ 当前群组没有绑定的会话，无法交还。");
          break;
        }
        const sessionId = ws.sessionId;

        // Do not hand back while a turn is running — would race the terminal.
        const session = this.sessionManager.getSession(chatId);
        if (session && session.getState() !== "idle") {
          await this.transport.sendText(chatId, "⚠️ 当前有任务正在执行，请等待完成或先 /stop 再交还。");
          break;
        }

        // Stop the agent owning the session and mark the group released.
